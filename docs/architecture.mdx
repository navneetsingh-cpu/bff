...
- When `AUTH_MODE=oidc`, the browser is then sent to Entra's
  `end_session_endpoint`. This matters more than it looks: without it, the user
  still has a live sign-in session *with Microsoft*, so clicking "sign in" again
  would put them straight back in with no prompt. To a user, that does not look
  like they were ever signed out.
